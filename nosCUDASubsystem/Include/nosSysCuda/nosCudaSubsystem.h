...
typedef void* nosCudaExternalSemaphore;

typedef void* nosCudaContext;

typedef void* nosCudaRandState;
...
{
	NOS_CUDA_COPY_KIND_HOST_TO_HOST = 0,
	NOS_CUDA_COPY_KIND_HOST_TO_DEVICE = 1,
	NOS_CUDA_COPY_KIND_DEVICE_TO_HOST = 2,
	NOS_CUDA_COPY_KIND_DEVICE_TO_DEVICE = 3,
	//TODO: Add MEMCPY_DEFAULT like CUDA			 
} nosCudaCopyKind;

typedef enum nosCudaMemoryType
{
	NOS_CUDA_MEMORY_TYPE_UNREGISTERED = 0, /**< Unregistered memory */
	NOS_CUDA_MEMORY_TYPE_HOST = 1, /**< Host memory */
	NOS_CUDA_MEMORY_TYPE_DEVICE = 2, /**< Device memory */
	NOS_CUDA_MEMORY_TYPE_MANAGED = 3 /**< Managed memory */
} nosCudaMemoryType;

typedef enum nosCudaEventFlags
{
	NOS_CUDA_EVENT_FLAG_DEFAULT = 0x00,
	NOS_CUDA_EVENT_FLAG_BLOCKING_SYNC = 0x01,
	NOS_CUDA_EVENT_FLAG_DISABLE_TIMING = 0x02,
} nosCudaEventFlags;

typedef enum nosCudaEventStatus
{
	NOS_CUDA_EVENT_STATUS_READY = 0,
	NOS_CUDA_EVENT_STATUS_NOT_READY = 1,
} nosCudaEventStatus;

typedef enum nosCudaExternalMemoryHandleType
{
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUEFD = 1,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUEWIN32 = 2,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUEWIN32KMT = 3,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12HEAP = 4,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12RESOURCE = 5,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11RESOURCE = 6,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11RESOURCEKMT = 7,
	NOS_CUDA_EXTERNAL_MEMORY_HANDLE_TYPE_NVSCIBUF = 8
} nosCudaExternalMemoryHandleType;

typedef enum nosCudaExternalSemaphoreHandleType
{
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUEFD = 1,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUEWIN32 = 2,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUEWIN32KMT = 3,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_D3D12FENCE = 4,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_D3D11FENCE = 5,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_NVSCISYNC = 6,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_KEYEDMUTEX = 7,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_KEYEDMUTEXKMT = 8,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_TIMELINESEMAPHOREFD = 9,
	NOS_CUDA_EXTERNAL_SEMAPHORE_HANDLE_TYPE_TIMELINESEMAPHOREWIN32 = 10
} nosCudaExternalSemaphoreHandleType;

typedef enum nosCudaContextFlags
{
	/// The default value if the flags parameter is zero, uses a heuristic based on the number of active CUDA contexts in the process C  
	...
	NOS_CUDA_CTX_FLAG_SCHED_AUTO = 0x00,
	
	/// Instruct CUDA to actively spin when waiting for results from the GPU. This can decrease latency when waiting 
	/// for the GPU, but may lower the performance of CPU threads if they are performing work in parallel with the CUDA thread.
	NOS_CUDA_CTX_FLAG_SCHED_SPIN = 0x01,

	/// Instruct CUDA to yield its thread when waiting for results from the GPU. This can increase latency when waiting 
	/// for the GPU, but can increase the performance of CPU threads performing work in parallel with the GPU.
	NOS_CUDA_CTX_FLAG_SCHED_YIELD = 0x02,

	/// Instruct CUDA to block the CPU thread on a synchronization primitive when waiting for the GPU to finish work
	NOS_CUDA_CTX_FLAG_SCHED_BLOCKING_SYNC = 0x04,
	NOS_CUDA_CTX_FLAG_SCHED_MASK = 0x07,
	NOS_CUDA_CTX_FLAG_LMEM_RESIZE_TO_MAX = 0X10,
	NOS_CUDA_CTX_FLAG_COREDUMP_ENABLE = 0x20,
	NOS_CUDA_CTX_FLAG_USER_COREDUMP_ENABLE = 0x40,

	/// Ensures that synchronous memory operations initiated on this context will always synchronize.
	NOS_CUDA_CTX_FLAG_SYNC_MEMOPS = 0x80,
} nosCudaContextFlags;

typedef enum nosCudaContextState
{
	NOS_CUDA_CTX_STATE_INACTIVE = 0,
	NOS_CUDA_CTX_STATE_ACTIVE = 1,
} nosCudaContextState;

typedef enum nosCudaResult
{
	NOS_CUDA_SUCCESS = 0,
	NOS_CUDA_ERROR_INVALID_VALUE = 1,
	NOS_CUDA_ERROR_MEMORY_ALLOCATION = 2,
	NOS_CUDA_ERROR_INITIALIZATION_ERROR = 3,
	...
	NOS_CUDA_ERROR_ALREADY_ACQUIRED = 210,
...
{
	nosVec3u GridDimensions;
	nosVec3u BlockDimensions;
	size_t DynamicMemorySize;
	//Default by zero, will be used for `extern __shared__ float shared[]` type dynamically allocated arrays.
...
typedef struct nosCudaSubsystem
{
	...
	//Sets the current context as the primary context of CUDA Subsystem
	nosResult (NOSAPI_CALL*GetCurrentContext)(nosCudaContext* cudaContext);
	//Retrieve the primary CUDA Context of CUDA Subsystem

	nosResult (NOSAPI_CALL*GetCudaVersion)(nosCudaVersion* versionInfo); //CUDA version
	...
	//device cant exceed deviceCount

	nosResult (NOSAPI_CALL*CreateStream)(nosObjectReference* outStreamObject);
	...
	//Loads .ptx files only
	nosResult (NOSAPI_CALL*LoadKernelModuleFromCString)(const char* ptxString,
	                                                    const char* errorLogBuffer,
	                                                    uint64_t errorLogBufferSize,
	                                                    nosCudaModule* outModule);
	//Loads kernels from CStrings (linking is not supported yet)
	...
	nosResult (NOSAPI_CALL*WaitStream)(nosCudaStreamObject stream); //Waits all commands in the stream to be completed
	nosCudaResult (NOSAPI_CALL*QueryStream)(nosCudaStreamObject stream); //

	nosCudaResult (NOSAPI_CALL*GetLastError)(); //Waits all commands in the stream to be completed

	nosResult (NOSAPI_CALL*AddEventToStream)(nosCudaStreamObject stream, nosCudaEvent event);
	...
	//Must be called before enqueueing the operation to stream
	nosResult (NOSAPI_CALL*GetCudaEventElapsedTime)(nosCudaStreamObject stream,
	                                                nosCudaEvent theEvent,
	                                                float* elapsedTime); //
	nosResult (NOSAPI_CALL*AddCallback)(nosCudaStreamObject stream,
	                                    nosCudaCallbackFunction callback,
	                                    void* callbackData);

	nosResult (NOSAPI_CALL*WaitExternalSemaphore)(nosCudaStreamObject stream,
	                                              nosCudaExternalSemaphore extSem,
	                                              uint64_t value);
	nosResult (NOSAPI_CALL*SignalExternalSemaphore)(nosCudaStreamObject stream,
	                                                nosCudaExternalSemaphore extSem,
	                                                uint64_t value);
	...
	nosResult (NOSAPI_CALL*CreateCuRandState)(nosCudaRandState* state, uint64_t count);
	nosResult (NOSAPI_CALL*DestroyCuRandState)(nosCudaRandState* state);

	nosResult (NOSAPI_CALL*GetStreamID)(nosCudaStreamObject stream, uint64_t* id);
...
#define NOS_CUDA_SUBSYSTEM_NAME "nos.sys.cuda"
#define NOS_CUDA_SUBSYSTEM_VERSION_MAJOR 3
#define NOS_CUDA_SUBSYSTEM_VERSION_MINOR 0

extern struct nosPluginInfo nosCudaPluginInfo;
extern nosCudaSubsystem* nosCuda;

#define NOS_CUDA_INIT()                                                                                                \
	nosPluginInfo nosCudaPluginInfo;																				   \
	nosCudaSubsystem* nosCuda = nullptr;

#define NOS_CUDA_IMPORT() NOS_IMPORT_DEP(NOS_CUDA_SUBSYSTEM_NAME, nosCudaPluginInfo, nosCuda)
